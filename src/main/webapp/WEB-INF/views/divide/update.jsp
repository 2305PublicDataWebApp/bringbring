...
<main style="width: 100%;margin: 0 auto;padding-top: 50px;">
	...
	<form name="updateForm" action="/divide/update.do" method="post" enctype="multipart/form-data">
			...
			<div style="margin: 0 auto;width: 800px;">
				<ul class="divide-insert-menu" style="padding: 0px;">
					<li>
						...
						<input name="divTitle" id="divTitle" type="text" id="title" value="${uData.divide.divTitle}">
					</li>
					...
					<li id="regionArea">
						...
						<select name="cityNo" id="cityNo" onchange="citySelect();" style="width: 100px;margin-right: 15px;">
							<c:forEach var="city" items="${cList}" >
								<c:if test="${city.cityNo eq  uData.divide.cityNo}">
									<option value="${city.cityNo}" selected>${city.cityName}</option>
								</c:if>
								<c:if test="${city.cityNo ne  uData.divide.cityNo}">
					...
					<li>
						...
						<select name="wasteCategoryNo" id="wasteCategoryNo" style="width: 176px;margin-right: 15px;">
							...
							<c:forEach var="wCategory" items="${wList}" >
								<c:if test="${wCategory.wasteCategoryNo eq  uData.wasteCategory.wasteCategoryNo}">
									<option value="${wCategory.wasteCategoryNo}" selected>${wCategory.wasteCategoryName}</option>
								</c:if>
								<c:if test="${wCategory.wasteCategoryNo ne  uData.wasteCategory.wasteCategoryNo}">
...
<script type="text/javascript">
	$(document).ready(function() {
		$.ajax({
			...
			data: { cityNo: ${uData.divide.cityNo} },
			type: "POST",
			success: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if(data[i].districtNo === ${uData.divide.districtNo}) {
