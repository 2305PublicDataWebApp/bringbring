      ...
      <div style="width: 1000px;margin: 0 auto;">
          <form action="/divide/search.do" method="get">
              <div class="input-group" style="width: 600px;float: left;" >
                  <select name="searchCondition" class="form-select" aria-label="Default select example" style="margin-right: 8px;border: 1px solid #adb5bd;">
                      <option value="all" <c:if test="${map.searchCondition eq 'all'}">selected</c:if>>전체</option>
                      <option value="title" <c:if test="${map.searchCondition eq 'title'}">selected</c:if>>제목</option>
                      <option value="region" <c:if test="${map.searchCondition eq 'region'}">selected</c:if>>지역</option>
                      <option value="category" <c:if test="${map.searchCondition eq 'category'}">selected</c:if>>카테고리</option>
                  </select>
                  <input value="${map.searchKeyword}" name="searchKeyword" style="width: 200px;border: 1px solid #adb5bd;" class="form-control me-2" type="search" placeholder="검색어를 입력해주세요." aria-label="Search">
          ...
          <div style="width: 100%;float: left;border-top: 1px solid #ccc;margin-top: 15px;padding: 10px 0px">
              ...
              <p style="margin: 0;padding: 7px 13px 0px 8px;margin-left: 3px;float: right;">
                  <c:if test="${map.searchOption eq 'date'}">
                      <a href="/divide/search.do?searchCondition=${map.searchCondition}&searchKeyword=${map.searchKeyword}" style="text-decoration:underline;text-underline-position:under;">최신순</a> |
                      <a href="/divide/search.do?searchOption=heart&searchCondition=${map.searchCondition}&searchKeyword=${map.searchKeyword}">추천순</a>
                  </c:if>
                  <c:if test="${map.searchOption ne 'date'}">
                      <a href="/divide/search.do?searchCondition=${map.searchCondition}&searchKeyword=${map.searchKeyword}">최신순</a> |
                      <a href="/divide/search.do?searchOption=heart&searchCondition=${map.searchCondition}&searchKeyword=${map.searchKeyword}" style="text-decoration:underline;text-underline-position:under;">추천순</a>
                  </c:if>
          </div>
...
<%--            정렬 최신순일 때--%>
            <c:if test="${map.searchOption eq 'date'}">
                <c:if test="${pInfo.startNavi ne 1}">
                    <c:url var="bPageUrl" value="/divide/search.do?searchCondition=${map.searchCondition}&searchKeyword=${map.searchKeyword}">
                ...
                <c:forEach begin="${pInfo.startNavi}" end="${pInfo.endNavi}" var="p">
                    <c:url var="pageUrl" value="/divide/search.do?searchCondition=${map.searchCondition}&searchKeyword=${map.searchKeyword}">
                ...
                <c:if test="${pInfo.endNavi ne pInfo.naviTotalCount}">
                    <c:url var="nPageUrl" value="/divide/search.do?searchCondition=${map.searchCondition}&searchKeyword=${map.searchKeyword}">
        ...
        <%--            정렬 하트순일 때--%>
            <c:if test="${map.searchOption ne 'date'}">
                <c:if test="${pInfo.startNavi ne 1}">
                    <c:url var="bPageUrl" value="/divide/search.do?searchOption=heart&searchCondition=${map.searchCondition}&searchKeyword=${map.searchKeyword}">
                        <c:param name="page" value="${pInfo.startNavi-1}"></c:param>
                    </c:url>
                    <li class="page-item">
                        <a style="color: black;" class="page-link" href="${bPageUrl}">Previous</a>
                ...
                <c:forEach begin="${pInfo.startNavi}" end="${pInfo.endNavi}" var="p">
                    <c:url var="pageUrl" value="/divide/search.do?searchOption=heart&searchCondition=${map.searchCondition}&searchKeyword=${map.searchKeyword}">
                        <c:param name="page" value="${p}"></c:param>
                    </c:url>
                    <li class="page-item">
                        <a style="color: black;" class="page-link" href="${pageUrl}">${p}</a>
                ...
                <c:if test="${pInfo.endNavi ne pInfo.naviTotalCount}">
                    <c:url var="nPageUrl" value="/divide/search.do?searchOption=heart&searchCondition=${map.searchCondition}&searchKeyword=${map.searchKeyword}">
                        <c:param name="page" value="${pInfo.endNavi+1}"></c:param>
                    </c:url>
                    <li class="page-item">
                        <a style="color: black;" class="page-link" href="${nPageUrl}">Next</a>
